...
Referer: https://auto.ru/sankt-peterburg/cars/new/?page=2
...
id = 1
for offer in data:
    salon = offer["salon"]["name"]
    price = offer["price_info"]["price"]
    mark = offer["vehicle_info"]["mark_info"]["name"]
    model = offer["vehicle_info"]["model_info"]["name"]
    salon_phone = offer["salon"]["phones"][0]["phone"]
    car_info = {"id":id , "diller_name":salon , "model":model, "mark": mark, "price":price}
    salon_info = salon + ',' + salon_phone
    cars.append(car_info)
    dillers.append(salon_info)
    id += 1

dillers = list(set(dillers))
id = 1
dillers_info = []
for diller in dillers:
    salon = diller.split(",")
    diller_info = {"id":id, "name":salon[0], "phone":salon[1]}
    dillers_info.append(diller_info)
    id += 1
